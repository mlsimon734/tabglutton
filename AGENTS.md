...
- Chrome reports **`url: ""` until a navigation commits**, keeping the target in the Chrome-only `pendingUrl`; Firefox fills `url` in immediately. A tab caught mid-load therefore looks address-less, which silently dropped it from `tabs_list` and — far worse — from the undo log, making that one close unreversible. `bridge-methods.ts` reads both through the `tabUrl` helper. Any new code reading `tab.url` on Chrome needs the same fallback.
